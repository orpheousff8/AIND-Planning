...
class PlanningGraph():
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        mutex_count = 0
        for a1_parent in node_s1.parents:
            for a2_parent in node_s2.parents:
                if a1_parent.is_mutex(a2_parent):
                    mutex_count += 1
        return (mutex_count == len(node_s1.parents))
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in self.problem.goal:
            found = False
            for idx, _ in enumerate(self.s_levels):
                if found:
                    break
                for s_node in self.s_levels[idx]:
